...
- 🎯 Entry points to execute your program using `python -m <flask_project_app>` or `$ flask_project_app` with basic CLI argument parsing.
...
---
# flask_project_app Flask Application

Awesome flask_project_app created by gassantos
...
```bash
git clone https://github.com/gassantos/flask-project-app flask_project_app
cd flask_project_app
make install
```
...
```bash
flask_project_app create-db   # run once
flask_project_app populate-db  # run once (optional)
flask_project_app add-user -u admin -p 1234  # ads a user
flask_project_app run
```
