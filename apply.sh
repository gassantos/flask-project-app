...
original_author="gassantos"
original_name="flask_project_app"
original_urlname="flask-project-app"
original_description="Awesome flask_project_app created by gassantos" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_project_app "${name}"
...
then
    ...
    echo "then run 'flask_project_app' or 'python -m flask_project_app'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_project_app' or 'python -m flask_project_app' or 'poetry run flask_project_app'"
fi
